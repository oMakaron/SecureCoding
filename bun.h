...
#define BUN_MAX_VIOLATIONS 128
#define BUN_VIOLATION_LEN  256

typedef struct {
    char         message[BUN_VIOLATION_LEN];
    bun_result_t result;
} BunViolation;
...
typedef struct {
    ...
    u64     file_size_u64;  // same value widened for overflow-safe arithmetic

    int     header_loaded;
    int     asset_table_range_ok;
    int     string_table_range_ok;
    int     data_section_range_ok;

    size_t  violation_count;
    size_t  malformed_count;
    size_t  unsupported_count;
    int     violation_overflow;
...
/**
 * Return the most appropriate parser result after validation. Malformed input
 * takes priority over unsupported optional features because malformed files are
 * unsafe to trust even if they also contain unsupported fields.
 */
bun_result_t bun_context_result(const BunParseContext *ctx);

/**
 * Print the violations collected during parsing, one per line.
 */
void bun_print_violations(FILE *out, const BunParseContext *ctx);

/**
 * Print as much human-readable BUN content as can safely be read.
 */
